...
if st.sidebar.button("🧹 RESET SYSTEM MONITOR"):
    ...
    st.success("Cleared back to $10.00 USD.")
    st.rerun()
...
# --- CLEAN LIVE SINGLE-STREAM PIPELINE FUNCTION ---
def fetch_live_market_tick(token, symbol, amount, strategy, run_trade=False):
    """Executes a clean, unblockable transactional check to get the exact real-time tick."""
    try:
        ...
        if token:
            ...
            if "error" not in auth_res and "authorize" in auth_res:
        ...
        return None, f"DISCONNECTED_RECONNECTING"
    return None, None

# --- ENGINE RE-RUN CONTROLLER ---
if st.session_state.engine_running:
    ...
    # Pull the exact real-time live tick data safely
    resp, result_data = fetch_live_market_tick(st.session_state.active_token, selected_symbol, standard_calculated_stake, "NEUTRAL")

    if isinstance(result_data, (int, float)):
        live_price_str = f"{result_data:.2f}"
        live_tick_digit = int(live_price_str[-1])
        confidence_signal = "📡 [CONNECTED: 100% UNBROKEN REAL-TIME PULSE LIVE]"
    else:
        # Graceful simulation fallback loop if server drops connection
        time.sleep(0.4)
        live_price_str = f"{random.uniform(750.00, 1250.00):.2f}"
        live_tick_digit = int(live_price_str[-1])
        confidence_signal = "⚠️ [RECONNECTING TO MATRIX CHANNELS...]"
    ...
    # --- MATHEMATICAL MATRIX EVALUATIONS ---
    ...
    with main_dashboard.container():
        ...
        st.code(
            f"=====================================================================================\n"
            f"⚡ CHITI ENGINE REAL-TIME COMPILER ONLINE\n"
            f"=====================================================================================\n"
            ...
            f"RECOMMENDED INTERCEPT ROUTE  : [{strategy_choice}] ({payout_multiplier:.2f}x)\n"
        ...
        if st.session_state.system_cooldown_active:
            ...
            if live_tick_digit == sniper_intercept_digit:
                ...
                if st.session_state.active_token:
                    trade_res, status_tag = fetch_live_market_tick(st.session_state.active_token, selected_symbol, active_stake, strategy_choice, run_trade=True)
                    st.session_state.total_trades += 1
                    st.session_state.current_trend_runs += 1
                    
                    if trade_res and "error" not in trade_res:
                        profit = float(trade_res["buy"].get("profit", 0))
                        if profit > 0:
                        ...
                        st.session_state.last_trade_status = f"❌ API EXCEPTION: {trade_res.get('error', {}).get('message', 'Network Timeout')}"
                else:
                    st.session_state.total_trades += 1
                    st.session_state.current_trend_runs += 1
    ...
    # Clean execution brake: prevents CPU overload and loops at exactly 1-second real market intervals
    time.sleep(0.6)
    st.rerun()
else:
